...
import { anyObj, ILTableInitOptions } from "../table";
...
export class Table extends CanvasCtx {
  ...
  constructor(dom: HTMLElement, options: ILTableInitOptions) {
    ...
    this.header = new TableHeader(this.ctx, {
      columns,
      cellH: colH,
      selectable,
    });
    ...
    this.scrollBar = new ScrollBar(this.ctx, colH, this.afterScroll.bind(this));
  ...
  setEventListener() {
    ...
    const mousemoveEvent = (event: MouseEvent) => {
      ...
      let { scrollTop, scrollLeft } = this.scrollBar;
      ...
      if (preAct !== activeRow) {
        ...
        if (!activeRow.active) activeRow.active = true;
      ...
      let activeCell = activeRow.cells.find((cell) => {
        ...
        return (
          offsetX + scrollLeft <= dertf(startX) + dertf(width) &&
          offsetX + scrollLeft > dertf(startX)
        );
      });
